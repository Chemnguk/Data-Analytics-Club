...
def extract_signal_data(instrument):
    ...
    print(f"Processing {instrument} signals...")
    for pid in tqdm(planet_ids):
...
# Extract and save both signal types
airs_signal = extract_signal_data("AIRS-CH0")
fgs_signal = extract_signal_data("FGS1")

print("✅ AIRS-CH0 and FGS1 signal files saved.")
...
def extract_calibration_data(instrument, calib_type):
    ...
    print(f"Processing {instrument} - {calib_type}...")
    for pid in tqdm(planet_ids):
